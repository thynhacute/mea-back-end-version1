...
export const isObjectFull = (val: any) => isObject(val) && hasLength(Object.keys(val));
...
export const isDateOnlyString = (val: any): boolean =>
    isStringFull(val) && /^\d{4}(-)(((0)[0-9])|((1)[0-2]))(-)([0-2][0-9]|(3)[0-1])$/g.test(val);
export const isTimeOnlyString = (val: any): boolean =>
    isStringFull(val) && /^([0-1]?[0-9]|2[0-3]):([0-5]?[0-9]):([0-5]?[0-9])$/g.test(val);
